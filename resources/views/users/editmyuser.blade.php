        ...
        <div class="card">
          <div class="card-body">
            ...
            @if (count($errors) > 0)
                    ...
                    @endif
                    <form method="POST" action="{{ route('updateuser', $professionalss->professional_id) }}" enctype="multipart/form-data">
            ...
            @csrf
                ...
                <img id="profile-picture" onclick="handleImageUpload()"  src="{{ asset('storage/' . $professionalss->professional_image) }}" alt="Uploaded Image">
                                    @endif
                                            <input id="profile-input" name="professional_image" type="file" accept="image/*" onchange="previewImage(event)">
                ...
                ?>
                           <select name="organization_name" class="form-select" id="organization_name" required>
    <option value="none">Select an organization</option>
    @foreach($organizations as $id => $name)
        <option value="{{ $id+1 }}" {{ $professionalss->organization_name === $name ? 'selected' : '' }}>{{ $name }}</option>
    @endforeach
</select>
