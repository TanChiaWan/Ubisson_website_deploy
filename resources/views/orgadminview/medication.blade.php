      ...
      <div class="container-fluid">
        ...
        <!-- Content -->
            ...
            <div class="row">
                ...
                <form action="{{ route('addallergyorg') }}" method="POST" style='display:inline-block;'>
                          @csrf
                  ...
                  <form action="{{ route('adddiagnosisorg') }}" method="POST" style='display:inline-block;'>
                          @csrf
                          <input type="hidden" name="patient_id" value="{{ $patientId }}" >
                        <input type="hidden" name="professional_id" value="{{ $professionalId }}">
                        <input type="hidden" name="organization_id" value="{{ $organizationid }}">
                ...
                <div class="modal-dialog modal-dialog-centered modal-sm">
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                      ...
                      <form action="{{ route('deleteallergyorg') }}" method="GET" style="display: inline-block;">
                          @csrf
                          <input type="hidden" name="allergy_id" value="" id="allergy_id_field"> <!-- This input will hold the allergy_id -->
...
$(document).ready(function () {
    ...
    var allergyId = link.data('allergyid'); // Get the allergy ID from the data attribute

    // Set the hidden input field's value with the allergy ID
    $('#allergy_id_field').val(allergyId);
});
});
</script>
                    ...
                    <div class="table-responsive table" style="overflow-y: hidden;">
                      <table class="table text-nowrap mb-0 align-middle datatable">
                        ...
                        <tr>
                          ...
                          <td class="border-bottom-0 align-middle"> <a href="javascript:void()" class="text-center delete-entry" data-bs-toggle="modal" data-bs-target="#delete_health_data_modal" data-allergyid="{{ $allergy->allergy_id }}"><i class="ti-trash ti"></i></a></td>
                        </tr>
                        @endforeach
...
<div class="modal fade" id="delete_medication_modal" tabindex="-1">
                <div class="modal-dialog modal-dialog-centered modal-sm">
                  <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-footer">
                      <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                      <form action="{{ route('deletemedicationorg') }}" method="GET" style="display: inline-block;">
                          @csrf
                          <input type="hidden" name="medication_id" value="" id="medication_id_field"> <!-- This input will hold the allergy_id -->
...
$(document).ready(function () {
    // When the modal is shown
    $('#delete_medication_modal').on('show.bs.modal', function (event) {
    var link = $(event.relatedTarget); // Get the "Delete" link that triggered the modal
    var medicationId = link.data('medicationid'); // Get the allergy ID from the data attribute

    // Set the hidden input field's value with the allergy ID
    $('#medication_id_field').val(medicationId);
});
});
</script>
   ...
   @php
    ...
    <!-- ... rest of the card content ... -->
        ...
        <div class="row">
          ...
          <form action="{{ route('patient-medicine-createorg') }}" method="POST" style="display:inline-block;">
    @csrf
    <input type="hidden" name="patient_id" value="{{ $patientId }}" >
                        <input type="hidden" name="professional_id" value="{{ $professionalId }}">
                        <input type="hidden" name="organization_id" value="{{ $organizationid }}">
...
<form action="{{ route('edit-diagnosisorg') }}" method="POST" style="display:inline-block;">
    @csrf
    <input type="hidden" name="patient_id" value="{{ $patientId }}" >
    <input type="hidden" name="professional_id" value="{{ $professionalId }}">
    <input type="hidden" name="organization_id" value="{{ $organizationid }}">
    ...
    <button type="submit" class="btn btn-primary m-2 btn-with-icon"><i class="ti ti-pencil"></i> Edit Diagnosis <i class="fas fa-edit"></i></button>
        ...
        @if ($medicationInDiagnosis->diagnosis_id == $singleDiagnosis->diagnosis_id)
            <tr>
                ...
                <td class="border-bottom-0">
                    @foreach(json_decode($medicationInDiagnosis->taken) as $takenTime)
                        <p class="mb-0 fw-normal">{{ $takenTime }}</p>
                    @endforeach
                </td>
...
@csrf
                ...
                <a href="javascript:void()" class="text-center delete-entry" data-bs-toggle="modal" data-bs-target="#delete_medication_modal" data-medicationid="{{ $medicationInDiagnosis->medication_id }}"><i class="ti-trash ti"></i></a>
