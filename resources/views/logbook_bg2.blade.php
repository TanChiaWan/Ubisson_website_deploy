            ...
            <div class="card-body">
                ...
                <div class="d-flex flex-row justify-content-between mt-2">
                  ...
                  <form action="{{ route('logbook_bg') }}" method="POST">
                        ...
                        <input type="hidden" name="patient_id" value="{{ $patient->patient_id }}" >
                        <input type="hidden" name="professional_id" value="{{ $user->professional_id }}">
                        <button type="submit"  class="btn btn-outline-secondary"  style="border-top-right-radius: 0;border-bottom-right-radius: 0;"><i class="ti ti-layout-list"></i></button>
                  </form>
                  <form action="{{ route('logbook_bg2') }}" method="POST">
                          @csrf
                        <input type="hidden" name="patient_id" value="{{ $patient->patient_id }}" >
                        <input type="hidden" name="professional_id" value="{{ $user->professional_id }}">
                        <button type="submit"  class="btn btn-outline-secondary active" style="border-top-left-radius: 0;border-bottom-left-radius: 0;"><i class="ti ti-table"></i></button>
                  </form>
                  </div>
                </div>
                <div class="tab_nav" >
